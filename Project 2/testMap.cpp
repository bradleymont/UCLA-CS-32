...
int main()
{
    ...
    assert(testString == "E" && testDouble == 50);
    ...
    //combine function with m1 and m2 having similar keys with conflicting values (with empty result)
    Map result4;
    assert(dups.update("A", 6));
    assert(!combine(originalOne, dups, result4));
    assert(result4.empty());
    
    //combine function with m1 and m2 having similar keys with conflicting values (with empty result)
    Map result5;
    assert(result5.insert("not empty", 2));
    assert(!combine(originalOne, dups, result5));   //result5 should remain untouched
    assert(result5.contains("not empty"));
    assert(result5.size() == 1);
    
    //subtract function with m1 != m2 and an empty result
    Map m1 = originalOne;
    Map m2 = originalTwo;
    Map res;
    subtract(m1, m2, res);
    assert(res.size() == m1.size());
    assert(res.contains("A"));
    assert(res.contains("F"));
    
    //subtract function with m1 != m2 and a non-empty result
    subtract(m1, m2, res);
    assert(res.size() == m1.size());
    assert(res.contains("A"));
    assert(res.contains("F"));
    ...
    //subtract function with m1 == m2 and a non-empty result
    subtract(m1, m1, res);
    assert(res.empty());   //res becomes empty
    
    //subtract function with m1 and m2 having similarities and an empty result
    Map m3;
    assert(m3.insert("A", 2));
    assert(m3.insert("F", 90));
    subtract(m1, m3, res);
    assert(!res.contains("A"));
    assert(!res.contains("F"));
    assert(res.size() == 4);
    
    //subtract function with m1 and m2 having similarities and a non-empty result
    subtract(m1, m3, res);
    assert(!res.contains("A"));
    assert(!res.contains("F"));
    assert(res.size() == 4);
    
    //subtract function where m1 = result (aliasing) and no similarities
    subtract(m1, m2, m1);
    assert(m1.size() == 6);
    ...
    //subtract function where m1 = result (aliasing) and similarities
    m2.insert("A", 2345);
    subtract(m1, m2, m1);
    assert(m1.size() == 5);
    assert(!m1.contains("A"));
